    ...
    <main className="min-h-screen px-6 py-12 sm:px-10 lg:px-16">
      ...
      <section className="glass-panel glow-cyan mt-8 rounded-2xl px-8 py-6">
        ...
        <div className="mt-6 flex flex-col gap-4">
          ...
          {downsyncRuns?.map((run) => (
            <DownsyncRunCard key={run.id} run={run} onDelete={() => setRunToDelete({ id: run.id, kind: 'downsync' })} />
          ))}
        </div>
      </section>
      ...
      <section className="glass-panel glow-violet mt-8 rounded-2xl px-8 py-6">
        ...
        <div className="mt-6 flex flex-col gap-4">
          ...
          {upsyncRuns?.map((run) => (
            <UpsyncRunCard key={run.id} run={run} onDelete={() => setRunToDelete({ id: run.id, kind: 'upsync' })} />
          ))}
        </div>
      </section>
